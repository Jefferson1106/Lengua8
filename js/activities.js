...
function modalSave() {
    if ($('#txtNota').html() == '') {
        mostrar_alerta('Califique la prueba antes de descargarla')
    } else {
        $('#myModal').modal('show')
    }
}
...
function calificar_abierta() {
    ...
    if (vacio > 0) {
        mostrar_alerta('todas las pregunatas se deben evaluar')
    }
